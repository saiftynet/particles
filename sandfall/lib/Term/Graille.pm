...
use base 'Exporter';
our @EXPORT_OK = qw/colour stripColours paint printAt clearScreen border blockBlit blankBlock block2braille pixelAt cursorAt wrapText border/;
...
sub set{
	...
	# the character is the last in a graille grid position
	# chop removes the character, leaving the colour in the grid position 
...
#	die "*".$self->{grid}->[$chrY]->[$chrX]."*".$self->{setPix}->[$xOffset]->[$yOffset]."*",$xOffset.",".$yOffset;
...
sub pixel{ #get pixel value at coordinates
	...
	#exit if out of bounds
	return 0 unless(($x<($self->{width}-1))&&($x>=0)&&($y<($self->{height}-1))&&($x>=0));
	my ($chrX,$chrY,$xOffset,$yOffset)=$self->charOffset($x,$y);
	my $bcr=stripColours($self->{grid}->[$chrY]->[$chrX]);
	$bcr='⠀' if (ord($bcr)&0x2800 !=0x2800); 
	my $orOp=ord($self->{setPix}-> [$xOffset]->[$yOffset]) & ord($bcr);
	return $orOp == ord('⠀')?0:1;
}
...
sub line{
	...
	Algorithm::Line::Bresenham::line($x1,$y1,$x2,$y2,$setRef,[$self,$value]);
}
...
sub blankBlock{ 
	my ($self, $blk)=@_;
	my $blank=[map{[('⠀') x scalar @{$blk->[0]}]} (0..$#$blk)];
	return $blank;
}
...
sub inGrid{
	...
	(($gridX>=0) && ($gridY>=0) && ($gridX<$self->{width}/2)  && ($gridY<$self->{height}/4))?1:0;
}
...
=head3 C<$canvas-E<gt>exportBlock($filename)> , C<$canvas-E<gt>importBlock($filename)>

This allows the loading and unloading of a block from a file. 
Export not yet added;

=cut

sub exportBlock{
	my ($self,$file,$x1,$y1, $x2, $y2)=@_;
	# placeholder
}
